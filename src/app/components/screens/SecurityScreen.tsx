                ...
                <div className="grid grid-cols-3 gap-4 w-full max-w-[260px]">
                  {PIN_KEYS.map((key, i) => (
                    <motion.button
                      ...
                      style={{ color: '#F1F5F9', fontSize: 22, fontWeight: 500, cursor: key === '' ? 'default' : 'pointer', border: '1px solid var(--border)' }}
                    >
    ...
    return (
      <div className="flex flex-col h-full" style={{ background: 'var(--background)' }}>
        ...
        <div className="flex-1 overflow-y-auto px-5">
          <div className="flex items-start gap-2 p-3 rounded-[12px] mb-4" style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.2)' }}>
          ...
          <motion.button whileTap={{ scale: 0.97 }} className="w-full py-3.5 rounded-[16px] text-white mb-3" style={{ background: 'var(--primary)', fontWeight: 700, fontSize: 15, boxShadow: '0 8px 24px rgba(99,102,241,0.4)' }}>
            Copy to Clipboard
          </motion.button>
        ...
        <div className="flex-1 overflow-y-auto px-5">
          {sessions.map((s, i) => (
            <div key={i} className="flex items-center gap-3 p-4 rounded-[16px] mb-3 glass-card" style={{ border: '1px solid var(--border)' }}>
              <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: s.current ? 'rgba(99,102,241,0.12)' : 'var(--muted)' }}>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {s.current && <span className="px-2 py-0.5 rounded-full" style={{ background: 'rgba(99,102,241,0.15)', color: 'var(--primary)', fontSize: 10, fontWeight: 700 }}>CURRENT</span>}
                </div>
                <p style={{ color: 'var(--muted-foreground)', fontSize: 12 }}>{s.location}</p>
                <p style={{ color: 'var(--muted-foreground)', fontSize: 11 }}>{s.time}</p>
              </div>
              {!s.current && (
                <motion.button whileTap={{ scale: 0.9 }} className="px-3 py-1.5 rounded-lg" style={{ background: 'rgba(239,68,68,0.1)', color: '#EF4444', fontSize: 12, fontWeight: 600 }}>
                  Revoke
                </motion.button>
              )}
            </div>
          ))}
          <motion.button whileTap={{ scale: 0.97 }} className="w-full py-3.5 rounded-[16px] mt-2" style={{ background: 'rgba(239,68,68,0.1)', color: '#EF4444', fontWeight: 700, fontSize: 15, border: '1px solid rgba(239,68,68,0.2)' }}>
      ...
      <div className="flex-1 overflow-y-auto px-5">
        <div className="rounded-[20px] p-5 mb-4 glass-card glass-refraction" style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.15), rgba(79,70,229,0.08))', border: '1px solid rgba(99,102,241,0.2)' }}>
          ...
          <div className="w-full h-2 rounded-full overflow-hidden" style={{ background: 'var(--muted)' }}>
            <motion.div initial={{ width: 0 }} animate={{ width: '85%' }} transition={{ duration: 0.8 }} className="h-full rounded-full" style={{ background: 'var(--primary)' }} />
          </div>
        </div>
