...
interface ReceiveSelectorsProps {
  asset: Asset;
  network: string;
  netInfo: NetworkInfo;
  onOpenAsset: () => void;
...
/** The asset card, "Change token" link, and network card above the QR code. */
export function ReceiveSelectors({ asset, network, netInfo, onOpenAsset, onOpenNetwork, onChangeToken }: ReceiveSelectorsProps) {
  return (
    <>
      <motion.button
        whileTap={{ scale: 0.99 }}
      ...
      <motion.button whileTap={{ scale: 0.97 }} onClick={onChangeToken} className="flex items-center gap-2 mb-4">
      ...
      >
        ...
        <div style={{ flex: 1, textAlign: 'left' }}>
          ...
          <p style={{ color: 'var(--muted-foreground)', fontSize: 12 }}>{netInfo.label} · ~{netInfo.estTime}</p>
        </div>
