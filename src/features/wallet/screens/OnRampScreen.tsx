...
import { ChevronLeft, Building, Phone, Clock, CheckCircle2, Loader, ArrowDownToLine, ChevronDown, CreditCard, Plus, Lock, Copy, Check } from 'lucide-react';
...
export function OnRampScreen({ goBack }: OnRampScreenProps) {
  ...
  const copyAccount = (text: string) => {
    try { navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 1500); } catch (e) { /* ignore */ }
  };
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                  ...
                  <AnimatePresence>
                    {showTokenDropdown && (
                      <motion.div key="dropdown" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="absolute top-full left-0 right-0 mt-1 rounded-[16px] overflow-hidden glass-card z-50" style={{ border: '1px solid var(--border)', boxShadow: '0 12px 40px rgba(0,0,0,0.4)' }}>
                        {rampAssets.map(asset => (
                          <button key={asset.id} onClick={() => { setSelectedAsset(asset); setShowTokenDropdown(false); }} className="w-full flex items-center gap-3 px-4 py-3" style={{ borderBottom: asset.id !== rampAssets[rampAssets.length - 1].id ? '1px solid var(--border)' : 'none' }}>
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2 mb-3 flex-wrap">
                  {(['bank', 'mobile', 'card'] as const).map(m => (
                    <button key={m} onClick={() => setPaymentMethod(m)} className="flex-1 min-w-[80px] flex items-center justify-center gap-1.5 py-2.5 rounded-[12px]" style={{ background: paymentMethod === m ? 'var(--primary)' : 'var(--card)', border: `1px solid ${paymentMethod === m ? 'transparent' : 'var(--border)'}`, color: paymentMethod === m ? '#FFF' : 'var(--muted-foreground)', fontSize: 13, fontWeight: 600 }}>
                ...
                {paymentMethod === 'card' && (
                  <div className="flex flex-col gap-2">
                    {cards.map(card => (
                      <button key={card.id} onClick={() => { setSelectedCardId(card.id); setShowNewCard(false); }} className="flex items-center gap-3 p-3.5 rounded-[14px]" style={{ background: selectedCardId === card.id && !showNewCard ? 'var(--muted)' : 'var(--card)', border: `1.5px solid ${selectedCardId === card.id && !showNewCard ? 'var(--primary)' : 'var(--border)'}` }}>
                    ...
                    {/* Add new card */}
                    <button onClick={() => { setShowNewCard(!showNewCard); setSelectedCardId(null); }} className="flex items-center gap-3 p-3.5 rounded-[14px]" style={{ background: showNewCard ? 'var(--muted)' : 'var(--card)', border: `1.5px solid ${showNewCard ? 'var(--primary)' : 'var(--border)'}` }}>
                    ...
                    {showNewCard && (
                        ...
                        <div className="p-4 rounded-[14px] glass-card" style={{ border: '1px solid var(--border)' }}>
                          ...
                          <motion.button whileTap={{ scale: 0.97 }} onClick={handleAddCard} className="w-full py-2.5 rounded-[10px] text-white" style={{ background: 'var(--primary)', fontSize: 13, fontWeight: 700 }}>Save Card and Use</motion.button>
                        </div>
                      </motion.div>
                    )}
                  </div>
              ...
              {amount && Number(amount) > 0 && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-[16px] mb-4" style={{ background: 'var(--muted)', border: '1px solid var(--border)' }}>
                  ...
                  <div className="flex justify-between"><span style={{ color: 'var(--foreground)', fontWeight: 700, fontSize: 14 }}>You Receive</span><span style={{ color: 'var(--foreground)', fontWeight: 800, fontSize: 18 }}>{youGet.toFixed(6)} {selectedAsset.symbol}</span></div>
                </motion.div>
              )}

              <motion.button whileTap={{ scale: 0.97 }} onClick={() => { if (Number(amount) > 0) setStep('review'); }} className="w-full py-3.5 rounded-[16px] text-white" style={{ background: Number(amount) > 0 ? 'var(--primary)' : 'var(--muted)', fontWeight: 700, fontSize: 15 }}>Preview On-Ramp</motion.button>
            </motion.div>
          )}
          ...
          {step === 'review' && (
            <motion.div key="review" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
              ...
              <motion.button whileTap={{ scale: 0.97 }} onClick={() => { setStep('payment-instructions'); }} className="w-full py-3.5 rounded-[16px] text-white" style={{ background: 'var(--primary)', fontWeight: 700, fontSize: 15 }}>Confirm & Get Payment Instructions</motion.button>
            </motion.div>
          )}
          ...
          {step === 'payment-instructions' && (
            <motion.div key="instructions" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
              <div className="rounded-[20px] p-5 mb-4 glass-card" style={{ border: '1px solid var(--border)' }}>
                ...
                <p style={{ color: 'var(--muted-foreground)', fontSize: 13, marginBottom: 16 }}>Send exactly <span style={{ color: 'var(--foreground)', fontWeight: 700 }}>{currency.symbol}{Number(amount).toLocaleString()}</span> to the account below to complete your on-ramp.</p>
                ...
                <div className="rounded-[14px] p-4 mb-4" style={{ background: 'var(--muted)', border: '1px solid var(--border)' }}>
                  {[
                    ...
                    <div key={row.label} className="flex items-center justify-between py-2.5" style={{ borderBottom: i < arr.length - 1 ? '1px solid var(--border)' : 'none' }}>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => copyAccount(row.value)} className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'var(--card)' }}>
              ...
              <motion.button whileTap={{ scale: 0.97 }} onClick={() => { setStep('processing'); setTimeout(() => setStep('done'), 3000); }} className="w-full py-3.5 rounded-[16px] text-white" style={{ background: 'var(--positive)', fontWeight: 700, fontSize: 15 }}>I've Made the Payment</motion.button>
            </motion.div>
          )}
          ...
          {step === 'processing' && (
            <motion.div key="processing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-20">
              ...
              <p style={{ color: 'var(--muted-foreground)', fontSize: 13, textAlign: 'center' }}>Converting {currency.symbol}{Number(amount).toLocaleString()} to {youGet.toFixed(6)} {selectedAsset.symbol}</p>
            </motion.div>
          )}
          ...
          {step === 'done' && (
            <motion.div key="done" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center py-12 text-center">
              ...
              <p style={{ color: 'var(--foreground)', fontSize: 28, fontWeight: 800, marginBottom: 4 }}>{youGet.toFixed(6)} {selectedAsset.symbol}</p>
              <p style={{ color: 'var(--muted-foreground)', fontSize: 13, marginBottom: 6 }}>Credited to your wallet</p>
              <div className="flex items-center gap-1.5 mb-10 px-3 py-1.5 rounded-full" style={{ background: 'var(--muted)' }}><Clock size={12} style={{ color: 'var(--foreground)' }} /><span style={{ color: 'var(--foreground)', fontSize: 12, fontWeight: 600 }}>Completed</span></div>
