...
import { TrendingUp, TrendingDown, Search, SlidersHorizontal, ChevronRight, Zap, CheckCircle2, Loader, Star, ChevronDown, X } from 'lucide-react';
import { AreaChart, Area, ResponsiveContainer, XAxis, YAxis, Tooltip, CartesianGrid } from 'recharts';
import { marketData, otcListings, portfolioChartData, cryptoAssets, type Screen } from '../../../shared/data/mockData';
...
export function TradeScreen({ navigate }: TradeScreenProps) {
  ...
  const effectivePrice = orderType === 'limit' && limitPrice ? Number(limitPrice) : selectedAsset.price;
  const cryptoAmount = usdAmount / effectivePrice;
    ...
    <div className="flex flex-col h-full overflow-y-auto" style={{ background: 'var(--background)' }}>
      ...
      <div className="flex items-center justify-between px-5 mb-4">
        ...
        <div className="flex gap-2">
          ...
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => navigate('swap')} className="w-10 h-10 rounded-2xl flex items-center justify-center" style={{ background: 'var(--secondary)' }}>
          ...
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <div className="flex items-center gap-1">
                ...
                <span style={{ color: 'var(--positive)', fontSize: 12, fontWeight: 600 }}>+2.52% this week</span>
              </div>
            </div>
          ...
          <div style={{ height: 120 }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <Area type="monotone" dataKey="value" stroke="var(--primary)" strokeWidth={2} fill="url(#tradeGrad)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {tradeTab === 'market' ? (
              ...
              <AnimatePresence mode="wait">
                  ...
                  <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    ...
                    <div className="flex gap-1 p-1 rounded-xl mb-4" style={{ background: 'var(--muted)' }}>
                      {(['market', 'limit'] as const).map(t => (
                        <button key={t} onClick={() => setOrderType(t)} className="flex-1 py-1.5 rounded-[8px] capitalize" style={{ background: orderType === t ? 'var(--card)' : 'transparent', color: orderType === t ? 'var(--foreground)' : 'var(--muted-foreground)', fontSize: 12, fontWeight: 600 }}>
                          {t} Order
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="mb-3 relative">
                      <p style={{ color: 'var(--muted-foreground)', fontSize: 12, marginBottom: 6 }}>Token</p>
                      <button onClick={() => setAssetDropdownOpen(o => !o)} className="w-full flex items-center justify-between px-4 py-3 rounded-[14px]" style={{ background: 'var(--muted)', border: '1px solid var(--border)' }}>
                        <div className="flex items-center gap-2.5">
                          <AssetIcon symbol={selectedAsset.symbol} size={28} />
                          <div className="text-left">
                            <p style={{ color: 'var(--foreground)', fontWeight: 700, fontSize: 14 }}>{selectedAsset.symbol}</p>
                            <p style={{ color: 'var(--muted-foreground)', fontSize: 11 }}>{selectedAsset.name}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <span style={{ color: 'var(--foreground)', fontSize: 13, fontWeight: 600 }}>{format(selectedAsset.price)}</span>
                      ...
                      <AnimatePresence>
                        {assetDropdownOpen && (
                          <>
                            ...
                            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="absolute top-full left-0 right-0 z-50 mt-1 rounded-[14px] overflow-hidden" style={{ background: 'var(--card)', border: '1px solid var(--border)', maxHeight: 280, overflowY: 'auto' }}>
                              {cryptoAssets.map(asset => (
                                <button key={asset.id} onClick={() => { setSelectedAsset(asset); setAssetDropdownOpen(false); setLimitPrice(''); }} className="w-full flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid var(--border)' }}>
                                  <div className="flex items-center gap-2.5">
                                  ...
                                  <div className="text-right">
                                    ...
                                    <p style={{ color: asset.change >= 0 ? 'var(--positive)' : 'var(--destructive)', fontSize: 11, fontWeight: 600 }}>{asset.change >= 0 ? '+' : ''}{asset.change}%</p>
                                  </div>
                                </button>
                              ))}
                            </motion.div>
                    ...
                    {orderType === 'limit' && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mb-3">
                        <p style={{ color: 'var(--muted-foreground)', fontSize: 12, marginBottom: 6 }}>Limit Price ({currency.code})</p>
                        <div className="flex items-center px-4 py-3 rounded-[14px]" style={{ background: 'var(--muted)', border: '1px solid var(--border)' }}>
                          ...
                          <input type="number" placeholder={String(selectedAsset.price)} value={limitPrice} onChange={e => setLimitPrice(e.target.value)} className="flex-1 bg-transparent outline-none" style={{ color: 'var(--foreground)', fontSize: 18, fontWeight: 600 }} />
                          <span style={{ color: 'var(--muted-foreground)', fontSize: 12 }}>per {selectedAsset.symbol}</span>
                        </div>
                        <div className="flex gap-2 mt-2">
                          {[-5, -2, 0, 2, 5].map(pct => {
                            const price = (selectedAsset.price * (1 + pct / 100)).toFixed(2);
                            return <button key={pct} onClick={() => setLimitPrice(price)} className="px-2.5 py-1 rounded-lg" style={{ background: 'var(--muted)', color: pct === 0 ? 'var(--foreground)' : pct > 0 ? 'var(--positive)' : 'var(--destructive)', fontSize: 11, fontWeight: 600 }}>{pct > 0 ? '+' : ''}{pct}%</button>;
                          })}
                        </div>
                      </motion.div>
                    )}
                    ...
                    <motion.button whileTap={{ scale: 0.97 }} onClick={handleTrade} className="w-full py-3.5 rounded-[14px] text-white" style={{ background: amount && usdAmount > 0 ? 'var(--primary)' : 'var(--muted)', fontWeight: 700, fontSize: 15, boxShadow: amount && usdAmount > 0 ? 'none' : 'none' }}>
                ...
                {step === 'review' && (
                  <motion.div key="review" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
                    ...
                    {[
                      { l: 'Order Type', v: orderType === 'market' ? 'Market Order' : 'Limit Order' },
                      { l: 'Price', v: `${format(effectivePrice)} / ${selectedAsset.symbol}` },
                      { l: 'Fee', v: format(fee) },
                    ...
                    <motion.button whileTap={{ scale: 0.97 }} onClick={confirmTrade} className="w-full py-3.5 rounded-[14px] text-white mt-4" style={{ background: 'var(--primary)', fontWeight: 700, fontSize: 15, boxShadow: 'none' }}>
                ...
                {step === 'processing' && (
                  <motion.div key="processing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center py-12">
                    ...
                    <p style={{ color: 'var(--muted-foreground)', fontSize: 13 }}>{orderType === 'limit' ? 'Placing limit order' : orderTab === 'buy' ? 'Buying' : 'Selling'} {orderType === 'limit' ? `at ${format(effectivePrice)}` : ''} {cryptoAmount.toFixed(6)} {selectedAsset.symbol}</p>
                  </motion.div>
                )}
                ...
                {step === 'done' && (
                  <motion.div key="done" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center py-8 text-center">
                    ...
                    <p style={{ color: 'var(--foreground)', fontSize: 24, fontWeight: 800, marginBottom: 4 }}>{cryptoAmount.toFixed(6)} {selectedAsset.symbol}</p>
          ...
          <div className="px-5 mb-4">
            ...
            <div className="rounded-[20px] overflow-hidden glass-card" style={{ border: '1px solid var(--border)' }}>
              ...
              {marketData.map((asset, i) => (
                <motion.div key={asset.symbol} whileTap={{ scale: 0.99 }} onClick={() => navigate('token-info', asset.symbol)} className="flex items-center px-4 py-3 cursor-pointer" style={{ borderBottom: i < marketData.length - 1 ? '1px solid var(--border)' : 'none' }}>
                  ...
                  <div style={{ width: 60, textAlign: 'right' }}>
                    <div className="flex items-center justify-end gap-1">
                      {asset.change >= 0 ? <TrendingUp size={11} style={{ color: 'var(--positive)' }} /> : <TrendingDown size={11} style={{ color: 'var(--destructive)' }} />}
                      <span style={{ color: asset.change >= 0 ? 'var(--positive)' : 'var(--destructive)', fontSize: 12, fontWeight: 600 }}>{Math.abs(asset.change)}%</span>
                    </div>
                  </div>
                </motion.div>
              ))}
        ...
        <div className="px-5 mb-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => navigate('otc')} className="flex items-center gap-1" style={{ color: 'var(--foreground)', fontSize: 13, fontWeight: 600 }}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="flex items-center gap-1">
                        ...
                        {listing.seller.online && <div className="w-2 h-2 rounded-full" style={{ background: 'var(--foreground)' }} />}
                      </div>
                      <p style={{ color: 'var(--muted-foreground)', fontSize: 11 }}>
                        <Star size={10} style={{ display: 'inline', color: 'var(--foreground)' }} /> {listing.seller.rating} · {listing.seller.trades} trades
                      </p>
                    </div>
                  </div>
                  <div className="px-2 py-1 rounded-lg" style={{ background: listing.type === 'buy' ? 'var(--muted)' : 'var(--muted)' }}>
                    <span style={{ color: listing.type === 'buy' ? 'var(--primary)' : 'var(--destructive)', fontSize: 12, fontWeight: 700, textTransform: 'uppercase' }}>{listing.type}</span>
                  </div>
                </div>
