...
/** Greeting + scan/notification buttons — name from live profile/session, never mock. */
export function HomeHeader({ notificationCount, onScan, onOpenNotifications }: HomeHeaderProps) {
  const { status, displayName: sessionName, username } = useAuth();
  ...
  const name =
    ...
    username ||
    (status === 'authenticated' ? (loading ? '…' : 'There') : 'Guest');
  ...
  return (
    <div className="flex items-center justify-between px-5 mb-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p style={{ color: 'var(--muted-foreground)', fontSize: 12 }}>{timeGreeting()}</p>
          <p style={{ color: 'var(--foreground)', fontWeight: 700, fontSize: 15, lineHeight: 1.2 }}>{name}</p>
        </div>
      </div>
      <div className="flex items-center gap-3">
        <motion.button
        ...
        >
          ...
          {notificationCount > 0 && (
            <span
              className="absolute -top-1 -right-1 w-5 h-5 rounded-full text-white flex items-center justify-center pulse-badge"
              style={{
                background: 'var(--destructive)',
                fontSize: 10,
                fontWeight: 700,
                boxShadow: '0 0 12px var(--muted)',
              }}
            >
              {notificationCount}
            </span>
