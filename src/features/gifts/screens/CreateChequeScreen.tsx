...
export function CreateChequeScreen({ goBack, navigate }: Props) {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto" style={{ background: 'var(--background)' }}>
      ...
      <div className="flex items-center gap-3 px-5 mb-5">
        ...
        <h1 style={{ color: 'var(--foreground)', fontWeight: 800, fontSize: 18 }}>Create cheque</h1>
      </div>

      <div className="px-5 pb-10 space-y-4">
        <Field label="Token">
          <div className="flex flex-wrap gap-2">
            {(tokens.length ? tokens : [{ symbol: 'USDT', name: 'Tether' } as any]).slice(0, 8).map((t: any) => {
        ...
        <Field label="Amount">
          ...
          {selected && (
            <p style={{ color: 'var(--muted-foreground)', fontSize: 12, marginTop: 6 }}>
              Balance {Number(selected.balance).toLocaleString()} {selected.symbol}
            </p>
          )}
        </Field>
