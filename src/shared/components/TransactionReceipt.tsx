...
export function TransactionReceipt({ tx, open, onClose }: TransactionReceiptProps) {
  ...
  const buildReceiptCanvas = () =>
    renderReceiptCanvas({
      ...
      amountLine: `${meta.sign}${formatTokenAmountPrecise(tx.amount)} ${tx.asset}`,
      secondaryLine: tx.assetTo ? `+${formatTokenAmountPrecise(tx.amountTo)} ${tx.assetTo}` : undefined,
            ...
            <div className="text-center px-5 mb-5 mt-2">
              ...
              {tx.assetTo && (
                <p style={{ color: 'var(--positive)', fontSize: 13, fontWeight: 600, marginTop: 2 }}>
                  +{formatTokenAmountPrecise(tx.amountTo)} {tx.assetTo}
                </p>
              )}
